...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        self.val_dataloader = StatefulDataLoader(
            ...
            # Validation datasets are sent to inference engines as a whole batch,
            # which will schedule the memory themselves.
            batch_size=self.config.data.get("val_batch_size", len(self.val_dataset)),
            num_workers=8,
            shuffle=False,
            drop_last=False,
            collate_fn=collate_fn,
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            for i, history in enumerate(test_output_gen_batch.non_tensor_batch['trajectories']):
                resp = ["# History {} #".format(i)]
                for message in history:
                    if message:
                        resp.append("# {}\n{}".format(message['role'], message['content']))
                resps.append("\n".join(resp))
            del test_output_gen_batch.non_tensor_batch['trajectories']
            response_lst.extend(test_output_gen_batch.non_tensor_batch['agent_average_response_len'])
            del test_output_gen_batch.non_tensor_batch['agent_average_response_len']
        ...
        log_text = "\n\n".join(resps)
        with open(f"outputs/{exp_name}/val/val_{self.global_steps}_log.txt", 'w') as f:
            f.write(log_text)
        ...
        metric_dict.update({"val_response_length/average_response_len_min": min(response_lst),
                            "val_response_length/average_response_len_mean": sum(response_lst)/len(response_lst),
                            "val_response_length/average_response_len_max": max(response_lst)})
        return metric_dict
    ...
    def fit(self):
        ...
        exp_name = self.config.trainer.experiment_name
        shutil.rmtree(f"outputs/{exp_name}/train", ignore_errors=True)
        os.makedirs(f"outputs/{exp_name}/train", exist_ok=True)
        shutil.rmtree(f"outputs/{exp_name}/val", ignore_errors=True)
        os.makedirs(f"outputs/{exp_name}/val", exist_ok=True)
        self.global_steps = 0
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            logger.logger['wandb'].save(f"outputs/{exp_name}/val/val_{self.global_steps}_log.txt")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    resps = []
                    for i, history in enumerate(gen_batch_output.non_tensor_batch['trajectories']):
                        resp = ["# History {} #".format(i)]
                        for message in history:
                            if message:
                                resp.append("# {}\n{}".format(message['role'], message['content']))
                        resps.append("\n".join(resp))
                    log_text = "\n\n".join(resps)
                    with open(f"outputs/{exp_name}/train/train_{self.global_steps}_log.txt", 'w') as f:
                        f.write(log_text)
                    logger.logger['wandb'].save(f"outputs/{exp_name}/train/train_{self.global_steps}_log.txt")
                    del gen_batch_output.non_tensor_batch['trajectories']
                    agent_average_response_len = gen_batch_output.non_tensor_batch['agent_average_response_len']
                    metrics.update({"response_length/agent_average_response_len_min": min(agent_average_response_len),
                                    "response_length/agent_average_response_len_mean": sum(agent_average_response_len)/len(agent_average_response_len),
                                    "response_length/agent_average_response_len_max": max(agent_average_response_len)})
                    del gen_batch_output.non_tensor_batch['agent_average_response_len']
                ...
                metrics.update(compute_train_metrics(batch=batch))

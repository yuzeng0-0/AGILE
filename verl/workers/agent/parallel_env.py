...
def _preprocess_multi_modal_inputs(prompt_str, processor, **kwargs):
    ...
    for img in input_mm_data["image"]:
        ...
        try:
            temp_image = Image.open(BytesIO(img_info["bytes"]))
        except:
            print("my_image_error"*100)
            print(f"Image type: {type(img)}")
            print(f"Image size: {img.size}")
            print(f"Image mode: {img.mode}")
            print(f"Image info: {img.info}")
            print(img_info["bytes"])
            print("my_image_error"*100)
            def create_white_image():
                """创建28x28白色RGB图像的PNG字节流"""
                img = Image.new('RGB', (28, 28), color=(255, 255, 255))
                buf = io.BytesIO()
                img.save(buf, format='PNG')
                return buf.getvalue()
            image_info_list[-1]['bytes'] = create_white_image()
...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params):
    ...
    trajectories_log = [
        []
        for prompt in prompts.non_tensor_batch['raw_prompt']
        for _ in range(sampling_params.n)
    ]
    agent_total_response_len = []
    agent_average_response_len = [0]*(batch_size*sampling_params.n)
    ...
    for i in range(batch_size):
        for _ in range(sampling_params.n):
            ...
            agent_total_response_len.append(0)
    ...
    for step in range(config.agent.max_turns):
        ...
        for idx, obs, act, rew, done in zip(active_indices, observations, actions, rewards, dones):
            ...
            trajectories_log[idx].append({'role': 'assistant',
                                          'content': act.outputs[0].text})
            if obs:
                obs_str = tokenizer.decode(obs['prompt_token_ids_vllm'])[len("\n<|im_start|>user\n"): -len("<|im_end|>\n<|im_start|>assistant\n")]
                trajectories_log[idx].append({'role': 'user',
                                            'content': obs_str})
            ...
            agent_total_response_len[idx] += len(act.outputs[0].token_ids)
            ...
            if running_states[idx].shape[-1] >= max_total_length or len(vllm_input_list[idx]['prompt_token_ids']) >= max_total_length \
                or done or step == config.agent.max_turns - 1:
                active_mask[idx] = False
                agent_average_response_len[idx] = agent_total_response_len[idx] / (step + 1)
                continue
    ...
    env.close()
    if np.array(trajectories_log, dtype=object).ndim == 2:
        trajectories_log[0].append({})
    ...
    return DataProto.from_dict(
        ...
        non_tensors={"multi_modal_inputs": mm_input_list,
                     ...
                     "agent_average_response_len": agent_average_response_len}if processor is not None else {
                        "trajectories": trajectories_log,
                        "agent_average_response_len": agent_average_response_len}
    )
